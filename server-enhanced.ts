 ...
 * 
 * Shows how to integrate the train detection system into your existing server.ts
 * Replace the existing server.ts code with this enhanced version.
 */
...
import {
	...
	debugEndpoint,
} from "./trainDetectionIntegration";
...
// Initialize train detection system on startup
initializeStationZones();
...
/**
 * Helper function to broadcast announcements via WebSocket
 */
function broadcastAnnouncementMessage(
	headcode: string,
	stationName: string,
): void {
	const message = {
		...
		headcode,
		stationName,
		announcement: `${headcode} is entering ${stationName}`,
...
async function positionsApi(context: Context) {
	...
	// ✨ NEW: Process train detection
	detectTrainsFromPlayerData(data.players);
...
// ✨ NEW: Debug endpoint to inspect detection system state
app.get("/api/train-detection/debug", (context) => {
	const debugState = trainDetectionSystem.getDebugState();
	return context.json(debugState);
});

// ✨ NEW: Endpoint to get announcement history
app.get("/api/train-detection/history", (context) => {
